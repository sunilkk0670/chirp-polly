...
async function uploadItalianA1Module09() {
    console.log('📤 Uploading Italian A1 Module 09 to Firestore...\n');

    try {
        const modulePath = join(__dirname, '..', 'firestore_data', 'ita_a1_m09.json');
        const moduleData = JSON.parse(readFileSync(modulePath, 'utf8'));

        const docRef = db.collection('languages').doc('italian')
            .collection('levels').doc('a1')
            .collection('modules').doc('ita_a1_m09');

        await docRef.set(moduleData);

        const wordCount = moduleData.lessons.reduce((sum, lesson) => sum + lesson.vocabularyItems.length, 0);

        console.log('✅ Successfully uploaded ita_a1_m09');
        console.log(`   Path: languages/italian/levels/a1/modules/ita_a1_m09`);
        ...
        console.log('   Current Module Count: 9');
        console.log('   Total Planned Modules: 10\n');

        console.log('🔍 Final Cumulative Audit (M01-M09)...\n');
        ...
        if (duplicates.length > 0) {
            ...
            console.log('✅ CLEAN DATABASE: 900 unique words across 9 modules.');
        }

        console.log('\n📊 Total unique words: ' + Object.keys(counts).length);
        console.log('📊 Total word instances: ' + allWords.length + '\n');

        console.log('📋 PROOF: Words 1-20 (Module 09):\n');
