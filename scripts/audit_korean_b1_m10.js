...
// Load M10
const m10Data = JSON.parse(fs.readFileSync(path.join(dataDir, 'korean_b1_m10.json'), 'utf8'));
const m10Words = m10Data.lessons.map(l => l.targetText);

console.log('--- 🧪 KOREAN B1 MODULE 10 AUDIT ---');

// 1. Internal Duplicates
const internalDups = m10Words.filter((w, i) => m10Words.indexOf(w) !== i);
...
// 2. Word Count
console.log(`✅ Word Count: ${m10Words.length}`);

// 3. Cumulative Overlaps
const masterSet = new Set([...a1Words, ...a2Words, ...b1Words]);
const overlaps = m10Words.filter(w => masterSet.has(w));

if (overlaps.length > 0) {
    console.log(`❌ Overlaps Found (${overlaps.length}):`, overlaps);
} else {
    console.log('✅ ZERO Cumulative Overlaps. Module 10 is 100% unique.');
}
