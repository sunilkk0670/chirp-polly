...
// Load M07
const m07Data = JSON.parse(fs.readFileSync(path.join(dataDir, 'korean_b1_m07.json'), 'utf8'));
const m07Words = m07Data.lessons.map(l => l.targetText);

console.log('--- 🧪 KOREAN B1 MODULE 07 AUDIT ---');

// 1. Internal Duplicates
const internalDups = m07Words.filter((w, i) => m07Words.indexOf(w) !== i);
...
// 2. Word Count
console.log(`✅ Word Count: ${m07Words.length}`);

// 3. Cumulative Overlaps
const masterSet = new Set([...a1Words, ...a2Words, ...b1Words]);
const overlaps = m07Words.filter(w => masterSet.has(w));

if (overlaps.length > 0) {
    console.log(`❌ Overlaps Found (${overlaps.length}):`, overlaps);
} else {
    console.log('✅ ZERO Cumulative Overlaps. Module 07 is 100% unique.');
}
