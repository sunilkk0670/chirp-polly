...
// Load previous A2 modules (M01-M07)
...
// Load current M08
const m08Data = JSON.parse(readFileSync(join(__dirname, '..', 'firestore_data', 'de_a2_m08.json'), 'utf8'));
const m08Items = m08Data.lessons[0].vocabularyItems;

// Intentional Review/Liar Game Traps (allowed to overlap)
const allowedTraps = new Set([
    'die miete', 'der rock', 'das gift', 'der chef', 'die pension'
]);

const overlaps = [];
m08Items.forEach(item => {
    const w = item.word.toLowerCase().trim();
...
console.log(`\n🔍 AUDIT RESULTS FOR German A2 M08:`);
console.log(`   Word Count: ${m08Items.length}`);
