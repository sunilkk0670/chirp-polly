...
// Load previous A2 modules (M01-M03)
...
// Load current M04
const m04Data = JSON.parse(readFileSync(join(__dirname, '..', 'firestore_data', 'de_a2_m04.json'), 'utf8'));
const m04Items = m04Data.lessons[0].vocabularyItems;

// Intentional Review/Liar Game Traps (allowed to overlap)
const allowedTraps = new Set([
    'die pension', 'das rathaus', 'der dom', 'der oldtimer', 'die autobahn',
    'hell', 'after', 'gift', 'sicher', 'das gesetz'
]);

const overlaps = [];
m04Items.forEach(item => {
    const w = item.word.toLowerCase().trim();
...
console.log(`\n🔍 AUDIT RESULTS FOR German A2 M04:`);
console.log(`   Word Count: ${m04Items.length}`);
