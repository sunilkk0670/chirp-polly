...
async function uploadKoreanA2Module3() {
    console.log('📤 Uploading Korean A2 Module 3 to Firestore...\n');

    try {
        const modulePath = join(__dirname, '..', 'firestore_data', 'ko_a2_m03.json');
        const moduleData = JSON.parse(readFileSync(modulePath, 'utf8'));

        const docRef = db.collection('languages').doc('korean')
            .collection('levels').doc('a2')
            .collection('modules').doc('ko_a2_m03');

        await docRef.set(moduleData);

        console.log('✅ Successfully uploaded ko_a2_m03');
        console.log(`   Path: languages/korean/levels/a2/modules/ko_a2_m03`);
