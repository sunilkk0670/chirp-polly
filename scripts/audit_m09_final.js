...
// Load previous A2 modules (M01-M08)
...
// Load current M09
const m09Data = JSON.parse(readFileSync(join(__dirname, '..', 'firestore_data', 'de_a2_m09.json'), 'utf8'));
const m09Items = m09Data.lessons[0].vocabularyItems;

// Intentional Review/Liar Game Traps (allowed to overlap)
const allowedTraps = new Set([
    'das stipendium', 'das gymnasium', 'die promotion', 'das protokoll'
]);

const overlaps = [];
m09Items.forEach(item => {
    const w = item.word.toLowerCase().trim();
...
console.log(`\n🔍 AUDIT RESULTS FOR German A2 M09:`);
console.log(`   Word Count: ${m09Items.length}`);
