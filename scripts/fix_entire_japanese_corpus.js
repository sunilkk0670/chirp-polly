...
const n3Pool = [
    ...
    { word: "勧告", reading: "kankoku", meaning: "Advice/Counsel", example_sentence: "正規勧告を受けるうちに、社内体制の不備が次々と露呈しました。" },
    ...
    { word: "強硬", reading: "kyoukou", meaning: "Firm/Strong/Rigid", example_sentence: "強硬な姿勢を崩さないうちに、事態は膠着状態に陥ってしまいました。" },
...
function getNextUnique() {
    while (poolIdx < n3Pool.length) {
        const item = n3Pool[poolIdx++];
        if (!masterSet.has(item.word)) {
...
allFiles.forEach(file => {
    ...
    // Process items and replace duplicates
    data.vocabularyItems = data.vocabularyItems.map(item => {
        if (masterSet.has(item.word)) {
            const repl = getNextUnique();
            if (repl) {
                console.log(`Global Fix: Replacing "${item.word}" in ${file} with "${repl.word}"`);
                return repl;
            } else {
                return item;
            }
    ...
    // Final count adjustment (target 100)
    while (data.vocabularyItems.length > 100) {
        data.vocabularyItems.pop();
    }
    while (data.vocabularyItems.length < 100) {
        const repl = getNextUnique();
        if (repl) data.vocabularyItems.push(repl);
    }

    fs.writeFileSync(filePath, JSON.stringify(data, null, 4));
});

console.log('Final Global Unique Words:', masterSet.size);
