...
async function uploadItalianA2Module01() {
    console.log('📤 Uploading Italian A2 Module 01 to Firestore...\n');

    try {
        const modulePath = join(__dirname, '..', 'firestore_data', 'ita_a2_m01.json');
        const moduleData = JSON.parse(readFileSync(modulePath, 'utf8'));

        const docRef = db.collection('languages').doc('italian')
            .collection('levels').doc('a2')
            .collection('modules').doc('ita_a2_m01');

        await docRef.set(moduleData);

        const wordCount = moduleData.lessons.reduce((sum, lesson) => sum + lesson.vocabularyItems.length, 0);

        console.log('✅ Successfully uploaded ita_a2_m01');
        console.log(`   Path: languages/italian/levels/a2/modules/ita_a2_m01`);
        ...
        console.log('🔍 Final Cumulative Audit (A1 M01-M10 + A2 M01)...\n');
        ...
        const moduleFiles = [
            ...
            'ita_a1_m10.json', 'ita_a2_m01.json'
        ];
        ...
        if (duplicates.length > 0) {
            ...
            console.log('✅ CLEAN DATABASE: 1100 unique words across 11 modules.');
        }

        console.log('\n📊 Total unique words: ' + Object.keys(counts).length);
        console.log('📊 Total word instances: ' + allWords.length + '\n');

        console.log('📋 PROOF: Words 1-20 (A2 Module 01):\n');
        ...
        console.log('\n📋 PROOF: Words 85-100 (A2 Module 01):\n');
