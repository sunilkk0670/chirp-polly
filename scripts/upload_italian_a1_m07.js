...
async function uploadItalianA1Module07() {
    console.log('📤 Uploading Italian A1 Module 07 to Firestore...\n');

    try {
        const modulePath = join(__dirname, '..', 'firestore_data', 'ita_a1_m07.json');
        const moduleData = JSON.parse(readFileSync(modulePath, 'utf8'));

        const docRef = db.collection('languages').doc('italian')
            .collection('levels').doc('a1')
            .collection('modules').doc('ita_a1_m07');

        await docRef.set(moduleData);

        const wordCount = moduleData.lessons.reduce((sum, lesson) => sum + lesson.vocabularyItems.length, 0);

        console.log('✅ Successfully uploaded ita_a1_m07');
        console.log(`   Path: languages/italian/levels/a1/modules/ita_a1_m07`);
        ...
        console.log('🔍 Final Cumulative Audit (M01-M07)...\n');
        ...
        if (duplicates.length > 0) {
            ...
            console.log('✅ CLEAN DATABASE: 700 unique words across 7 modules.');
        }

        console.log('\n📊 Total unique words: ' + Object.keys(counts).length);
        console.log('📊 Total word instances: ' + allWords.length + '\n');

        console.log('📋 PROOF: Words 1-20 (Module 07):\n');
