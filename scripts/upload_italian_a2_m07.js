...
async function uploadItalianA2Module07() {
    console.log('📤 Uploading Italian A2 Module 07 to Firestore...\n');

    try {
        const modulePath = join(__dirname, '..', 'firestore_data', 'ita_a2_m07.json');
        const moduleData = JSON.parse(readFileSync(modulePath, 'utf8'));

        const docRef = db.collection('languages').doc('italian')
            .collection('levels').doc('a2')
            .collection('modules').doc('ita_a2_m07');

        await docRef.set(moduleData);

        const wordCount = moduleData.lessons.reduce((sum, lesson) => sum + lesson.vocabularyItems.length, 0);

        console.log('✅ Successfully uploaded ita_a2_m07');
        console.log(`   Path: languages/italian/levels/a2/modules/ita_a2_m07`);
        ...
        console.log('   Current Module Count: 7');
        console.log('   Total Planned Modules: 10\n');

        console.log('🔍 Final Cumulative Audit (A1 M01-M10 + A2 M01-M07)...\n');
        ...
        const moduleFiles = [
            ...
            'ita_a2_m03.json', 'ita_a2_m04.json', 'ita_a2_m05.json',
            'ita_a2_m06.json', 'ita_a2_m07.json'
        ];
        ...
        moduleFiles.forEach(file => {
            ...
            if (existsSync(p)) {
                ...
                data.lessons.forEach(l => l.vocabularyItems.forEach(i => {
                    // Normalize: lowercase, trim, remove definite/indefinite articles for matching
                    const w = i.word.toLowerCase().trim().replace(/^(il|la|lo|i|le|gli|un|una|un\x27|uno|l\x27)\s+|^(l\x27)/, "");
                    counts[w] = (counts[w] || 0) + 1;
                    allWords.push({ word: i.word, module: file, norm: w });
                }));
            }
        });
        ...
        if (duplicates.length > 0) {
            ...
            duplicates.forEach(([word, count]) => {
                const modules = allWords.filter(w => w.norm === word).map(w => w.module);
                console.log(`   - "${word}" appears ${count} times in: ${[...new Set(modules)].join(', ')}`);
            });
        } else {
            console.log('✅ CLEAN DATABASE: 1700 unique words across 17 modules.');
        }

        console.log('\n📊 Total unique words: ' + Object.keys(counts).length);
        console.log('📊 Total word instances: ' + allWords.length + '\n');

        console.log('📋 PROOF: Words 1-20 (A2 Module 07):\n');
        ...
        console.log('\n📋 PROOF: Words 85-100 (A2 Module 07):\n');
